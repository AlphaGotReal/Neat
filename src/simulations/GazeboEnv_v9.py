...
class GazeboEnv(gym.Env):
    def __init__(self) -> None:
        ...
        self.marker.scale.x = self.goal_threshold
        self.marker.scale.y = self.goal_threshold
        self.marker.scale.z = self.goal_threshold
    ...
    def step(self, action):
        ...
        if(done == True):
            self.gen_goal(pos_x, pos_y, 3, (-np.pi, np.pi))
            self.prev_goal_distance = np.sqrt((self.goal_x)**2 + (self.goal_y)**2)
    ...
    def gen_goal(self, pos_x, pos_y, distance, theta_range):
        ...
        distance = np.random.uniform(1.5, 3.0 - 0.3) + np.random.choice([0, 0.3, 0.6])
        x = 0 + distance * math.cos(theta)
        y = 0 + distance * math.sin(theta)

        self.lost_threshold = 2*distance
    ...
    def gen_reward(self, robo_pos_x, robo_pos_y, orientation_z, orientation_w):
        ...
        reward = 1.1 if reached else (-0.1 if abs(dtheta) <= abs(self.ideal_dtheta) else -1)
        self.ideal_dtheta = dtheta
        #Defining distance_from_shortest_path reward function
        # if reached:
        #     reward = 1
        # if distance_from_shortest_path < self.prev_distance_from_line:
        #     reward = reward + 0.01 * (self.prev_distance_from_line - distance_from_shortest_path)
        # elif distance_from_shortest_path > self.prev_distance_from_line:
        #     reward = reward - 0.2 * (distance_from_shortest_path - self.prev_distance_from_line)
        # else:
        #     reward = -0.1
